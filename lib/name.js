...
exports.name = {
  ...
  run: async (context, category, sex) => {
    switch (category) {
      case "firstName": return faker.name.firstName(sex);
      case "middleName": return faker.name.middleName(sex);
      case "lastName": return faker.name.lastName(sex);
      case "fullName": return faker.name.fullName({ sex: sex });
      case "prefix": return faker.name.prefix(sex);
      case "suffix": return faker.name.suffix();
      default: return "UNKNOWN-NAME-OPTION";
    }
  }
};
