exports.all_country_codes = [
    "AF", "AX", "AL", "DZ", "AS", "AD", "AO", "AI", "AQ", "AG", "AR", "AM", "AW", "AU", "AT", "AZ", "BS", "BH",
    "BD", "BB", "BY", "BE", "BZ", "BJ", "BM", "BT", "BO", "BQ", "BA", "BW", "BV", "BR", "IO", "VG", "BN", "BG",
    "BF", "BI", "CV", "KH", "CM", "CA", "KY", "CF", "TD", "CL", "CN", "CX", "CC", "CO", "KM", "CK", "CR", "HR",
    "CU", "CW", "CY", "CZ", "CD", "DK", "DJ", "DM", "DO", "EC", "EG", "SV", "GQ", "ER", "EE", "SZ", "ET", "FK",
    "FO", "FJ", "FI", "FR", "GF", "PF", "TF", "GA", "GM", "GE", "DE", "GH", "GI", "GR", "GL", "GD", "GP", "GU",
    "GT", "GG", "GN", "GW", "GY", "HT", "HM", "HN", "HK", "HU", "IS", "IN", "ID", "IR", "IQ", "IE", "IM", "IL",
    "IT", "CI", "JM", "JP", "JE", "JO", "KZ", "KE", "KI", "XK", "KW", "KG", "LA", "LV", "LB", "LS", "LR", "LY",
    "LI", "LT", "LU", "MO", "MG", "MW", "MY", "MV", "ML", "MT", "MH", "MQ", "MR", "MU", "YT", "MX", "FM", "MD",
    "MC", "MN", "ME", "MS", "MA", "MZ", "MM", "NA", "NR", "NP", "NL", "AN", "NC", "NZ", "NI", "NE", "NG", "NU",
    "NF", "KP", "MK", "MP", "NO", "OM", "PK", "PW", "PS", "PA", "PG", "PY", "PE", "PH", "PN", "PL", "PT", "PR",
    "QA", "CG", "RE", "RO", "RU", "RW", "BL", "SH", "KN", "LC", "MF", "PM", "VC", "WS", "SM", "ST", "SA", "SN",
    "RS", "CS", "SC", "SL", "SG", "SX", "SK", "SI", "SB", "SO", "ZA", "GS", "KR", "SS", "ES", "LK", "SD", "SR",
    "SJ", "SE", "CH", "SY", "TW", "TJ", "TZ", "TH", "TL", "TG", "TK", "TO", "TT", "TN", "TR", "TM", "TC", "TV",
    "VI", "UG", "UA", "AE", "GB", "US", "UM", "UY", "UZ", "VU", "VA", "VE", "VN", "WF", "EH", "YE", "ZM", "ZW"
]

const all_countries = {
    "AD": {
        "name": "Andorra",
        "postal_code_format": "AD###"
    },
    "AE": {
        "name": "United Arab Emirates",
        "postal_code_format": "#####"
    },
    "AF": {
        "name": "Afghanistan",
        "postal_code_format": "#####"
    },
    "AG": {
        "name": "Antigua and Barbuda",
        "postal_code_format": "#####"
    },
    "AI": {
        "name": "Anguilla",
        "postal_code_format": "#####"
    },
    "AL": {
        "name": "Albania",
        "postal_code_format": "####"
    },
    "AM": {
        "name": "Armenia",
        "postal_code_format": "######"
    },
    "AN": {
        "name": "Netherlands Antilles",
        "postal_code_format": "#####"
    },
    "AO": {
        "name": "Angola",
        "postal_code_format": "#####"
    },
    "AQ": {
        "name": "Antarctica",
        "postal_code_format": "#####"
    },
    "AR": {
        "name": "Argentina",
        "postal_code_format": "?####???"
    },
    "AS": {
        "name": "American Samoa",
        "postal_code_format": "#####-####"
    },
    "AT": {
        "name": "Austria",
        "postal_code_format": "####"
    },
    "AU": {
        "name": "Australia",
        "postal_code_format": "####"
    },
    "AW": {
        "name": "Aruba",
        "postal_code_format": "#####"
    },
    "AX": {
        "name": "Aland Islands",
        "postal_code_format": "#####"
    },
    "AZ": {
        "name": "Azerbaijan",
        "postal_code_format": "AZ ####"
    },
    "BA": {
        "name": "Bosnia and Herzegovina",
        "postal_code_format": "#####"
    },
    "BB": {
        "name": "Barbados",
        "postal_code_format": "BB#####"
    },
    "BD": {
        "name": "Bangladesh",
        "postal_code_format": "####"
    },
    "BE": {
        "name": "Belgium",
        "postal_code_format": "####"
    },
    "BF": {
        "name": "Burkina Faso",
        "postal_code_format": "#####"
    },
    "BG": {
        "name": "Bulgaria",
        "postal_code_format": "####"
    },
    "BH": {
        "name": "Bahrain",
        "postal_code_format": "####"
    },
    "BI": {
        "name": "Burundi",
        "postal_code_format": "#####"
    },
    "BJ": {
        "name": "Benin",
        "postal_code_format": "#####"
    },
    "BL": {
        "name": "Saint Barthelemy",
        "postal_code_format": "#####"
    },
    "BM": {
        "name": "Bermuda",
        "postal_code_format": "?? ##"
    },
    "BN": {
        "name": "Brunei",
        "postal_code_format": "??####"
    },
    "BO": {
        "name": "Bolivia",
        "postal_code_format": "#####"
    },
    "BQ": {
        "name": "Bonaire, Saint Eustatius and Saba ",
        "postal_code_format": "#####"
    },
    "BR": {
        "name": "Brazil",
        "postal_code_format": "#####-###"
    },
    "BS": {
        "name": "Bahamas",
        "postal_code_format": "#####"
    },
    "BT": {
        "name": "Bhutan",
        "postal_code_format": "#####"
    },
    "BV": {
        "name": "Bouvet Island",
        "postal_code_format": "#####"
    },
    "BW": {
        "name": "Botswana",
        "postal_code_format": "#####"
    },
    "BY": {
        "name": "Belarus",
        "postal_code_format": "######"
    },
    "BZ": {
        "name": "Belize",
        "postal_code_format": "#####"
    },
    "CA": {
        "name": "Canada",
        "postal_code_format": "?#? #?#"
    },
    "CC": {
        "name": "Cocos Islands",
        "postal_code_format": "#####"
    },
    "CD": {
        "name": "Democratic Republic of the Congo",
        "postal_code_format": "#####"
    },
    "CF": {
        "name": "Central African Republic",
        "postal_code_format": "#####"
    },
    "CG": {
        "name": "Republic of the Congo",
        "postal_code_format": "#####"
    },
    "CH": {
        "name": "Switzerland",
        "postal_code_format": "####"
    },
    "CI": {
        "name": "Ivory Coast",
        "postal_code_format": "#####"
    },
    "CK": {
        "name": "Cook Islands",
        "postal_code_format": "#####"
    },
    "CL": {
        "name": "Chile",
        "postal_code_format": "#######"
    },
    "CM": {
        "name": "Cameroon",
        "postal_code_format": "#####"
    },
    "CN": {
        "name": "China",
        "postal_code_format": "######"
    },
    "CO": {
        "name": "Colombia",
        "postal_code_format": "######"
    },
    "CR": {
        "name": "Costa Rica",
        "postal_code_format": "#####"
    },
    "CS": {
        "name": "Serbia and Montenegro",
        "postal_code_format": "#####"
    },
    "CU": {
        "name": "Cuba",
        "postal_code_format": "CP #####"
    },
    "CV": {
        "name": "Cabo Verde",
        "postal_code_format": "####"
    },
    "CW": {
        "name": "Curacao",
        "postal_code_format": "#####"
    },
    "CX": {
        "name": "Christmas Island",
        "postal_code_format": "####"
    },
    "CY": {
        "name": "Cyprus",
        "postal_code_format": "####"
    },
    "CZ": {
        "name": "Czechia",
        "postal_code_format": "### ##"
    },
    "DE": {
        "name": "Germany",
        "postal_code_format": "#####"
    },
    "DJ": {
        "name": "Djibouti",
        "postal_code_format": "#####"
    },
    "DK": {
        "name": "Denmark",
        "postal_code_format": "####"
    },
    "DM": {
        "name": "Dominica",
        "postal_code_format": "#####"
    },
    "DO": {
        "name": "Dominican Republic",
        "postal_code_format": "#####"
    },
    "DZ": {
        "name": "Algeria",
        "postal_code_format": "#####"
    },
    "EC": {
        "name": "Ecuador",
        "postal_code_format": "?####?"
    },
    "EE": {
        "name": "Estonia",
        "postal_code_format": "#####"
    },
    "EG": {
        "name": "Egypt",
        "postal_code_format": "#####"
    },
    "EH": {
        "name": "Western Sahara",
        "postal_code_format": "#####"
    },
    "ER": {
        "name": "Eritrea",
        "postal_code_format": "#####"
    },
    "ES": {
        "name": "Spain",
        "postal_code_format": "#####"
    },
    "ET": {
        "name": "Ethiopia",
        "postal_code_format": "####"
    },
    "FI": {
        "name": "Finland",
        "postal_code_format": "#####"
    },
    "FJ": {
        "name": "Fiji",
        "postal_code_format": "#####"
    },
    "FK": {
        "name": "Falkland Islands",
        "postal_code_format": "#####"
    },
    "FM": {
        "name": "Micronesia",
        "postal_code_format": "#####"
    },
    "FO": {
        "name": "Faroe Islands",
        "postal_code_format": "###"
    },
    "FR": {
        "name": "France",
        "postal_code_format": "#####"
    },
    "GA": {
        "name": "Gabon",
        "postal_code_format": "#####"
    },
    "GB": {
        "name": "United Kingdom",
        "postal_code_format": "??## #??"
    },
    "GD": {
        "name": "Grenada",
        "postal_code_format": "#####"
    },
    "GE": {
        "name": "Georgia",
        "postal_code_format": "####"
    },
    "GF": {
        "name": "French Guiana",
        "postal_code_format": "#####"
    },
    "GG": {
        "name": "Guernsey",
        "postal_code_format": "??## #??"
    },
    "GH": {
        "name": "Ghana",
        "postal_code_format": "#####"
    },
    "GI": {
        "name": "Gibraltar",
        "postal_code_format": "#####"
    },
    "GL": {
        "name": "Greenland",
        "postal_code_format": "####"
    },
    "GM": {
        "name": "Gambia",
        "postal_code_format": "#####"
    },
    "GN": {
        "name": "Guinea",
        "postal_code_format": "#####"
    },
    "GP": {
        "name": "Guadeloupe",
        "postal_code_format": "#####"
    },
    "GQ": {
        "name": "Equatorial Guinea",
        "postal_code_format": "#####"
    },
    "GR": {
        "name": "Greece",
        "postal_code_format": "### ##"
    },
    "GS": {
        "name": "South Georgia and the South Sandwich Islands",
        "postal_code_format": "#####"
    },
    "GT": {
        "name": "Guatemala",
        "postal_code_format": "#####"
    },
    "GU": {
        "name": "Guam",
        "postal_code_format": "969##"
    },
    "GW": {
        "name": "Guinea-Bissau",
        "postal_code_format": "####"
    },
    "GY": {
        "name": "Guyana",
        "postal_code_format": "#####"
    },
    "HK": {
        "name": "Hong Kong",
        "postal_code_format": "#####"
    },
    "HM": {
        "name": "Heard Island and McDonald Islands",
        "postal_code_format": "#####"
    },
    "HN": {
        "name": "Honduras",
        "postal_code_format": "??####"
    },
    "HR": {
        "name": "Croatia",
        "postal_code_format": "#####"
    },
    "HT": {
        "name": "Haiti",
        "postal_code_format": "HT####"
    },
    "HU": {
        "name": "Hungary",
        "postal_code_format": "####"
    },
    "ID": {
        "name": "Indonesia",
        "postal_code_format": "#####"
    },
    "IE": {
        "name": "Ireland",
        "postal_code_format": "??? ????"
    },
    "IL": {
        "name": "Israel",
        "postal_code_format": "#######"
    },
    "IM": {
        "name": "Isle of Man",
        "postal_code_format": "??## #??"
    },
    "IN": {
        "name": "India",
        "postal_code_format": "######"
    },
    "IO": {
        "name": "British Indian Ocean Territory",
        "postal_code_format": "#####"
    },
    "IQ": {
        "name": "Iraq",
        "postal_code_format": "#####"
    },
    "IR": {
        "name": "Iran",
        "postal_code_format": "##########"
    },
    "IS": {
        "name": "Iceland",
        "postal_code_format": "###"
    },
    "IT": {
        "name": "Italy",
        "postal_code_format": "#####"
    },
    "JE": {
        "name": "Jersey",
        "postal_code_format": "??## #??"
    },
    "JM": {
        "name": "Jamaica",
        "postal_code_format": "#####"
    },
    "JO": {
        "name": "Jordan",
        "postal_code_format": "#####"
    },
    "JP": {
        "name": "Japan",
        "postal_code_format": "###-####"
    },
    "KE": {
        "name": "Kenya",
        "postal_code_format": "#####"
    },
    "KG": {
        "name": "Kyrgyzstan",
        "postal_code_format": "######"
    },
    "KH": {
        "name": "Cambodia",
        "postal_code_format": "#####"
    },
    "KI": {
        "name": "Kiribati",
        "postal_code_format": "#####"
    },
    "KM": {
        "name": "Comoros",
        "postal_code_format": "#####"
    },
    "KN": {
        "name": "Saint Kitts and Nevis",
        "postal_code_format": "#####"
    },
    "KP": {
        "name": "North Korea",
        "postal_code_format": "###-###"
    },
    "KR": {
        "name": "South Korea",
        "postal_code_format": "#####"
    },
    "KW": {
        "name": "Kuwait",
        "postal_code_format": "#####"
    },
    "KY": {
        "name": "Cayman Islands",
        "postal_code_format": "#####"
    },
    "KZ": {
        "name": "Kazakhstan",
        "postal_code_format": "######"
    },
    "LA": {
        "name": "Laos",
        "postal_code_format": "#####"
    },
    "LB": {
        "name": "Lebanon",
        "postal_code_format": "#### ####"
    },
    "LC": {
        "name": "Saint Lucia",
        "postal_code_format": "#####"
    },
    "LI": {
        "name": "Liechtenstein",
        "postal_code_format": "####"
    },
    "LK": {
        "name": "Sri Lanka",
        "postal_code_format": "#####"
    },
    "LR": {
        "name": "Liberia",
        "postal_code_format": "####"
    },
    "LS": {
        "name": "Lesotho",
        "postal_code_format": "###"
    },
    "LT": {
        "name": "Lithuania",
        "postal_code_format": "LT-#####"
    },
    "LU": {
        "name": "Luxembourg",
        "postal_code_format": "L-####"
    },
    "LV": {
        "name": "Latvia",
        "postal_code_format": "LV-####"
    },
    "LY": {
        "name": "Libya",
        "postal_code_format": "#####"
    },
    "MA": {
        "name": "Morocco",
        "postal_code_format": "#####"
    },
    "MC": {
        "name": "Monaco",
        "postal_code_format": "#####"
    },
    "MD": {
        "name": "Moldova",
        "postal_code_format": "MD-####"
    },
    "ME": {
        "name": "Montenegro",
        "postal_code_format": "#####"
    },
    "MF": {
        "name": "Saint Martin",
        "postal_code_format": "#####"
    },
    "MG": {
        "name": "Madagascar",
        "postal_code_format": "###"
    },
    "MH": {
        "name": "Marshall Islands",
        "postal_code_format": "#####-####"
    },
    "MK": {
        "name": "North Macedonia",
        "postal_code_format": "####"
    },
    "ML": {
        "name": "Mali",
        "postal_code_format": "#####"
    },
    "MM": {
        "name": "Myanmar",
        "postal_code_format": "#####"
    },
    "MN": {
        "name": "Mongolia",
        "postal_code_format": "######"
    },
    "MO": {
        "name": "Macao",
        "postal_code_format": "#####"
    },
    "MP": {
        "name": "Northern Mariana Islands",
        "postal_code_format": "#####"
    },
    "MQ": {
        "name": "Martinique",
        "postal_code_format": "#####"
    },
    "MR": {
        "name": "Mauritania",
        "postal_code_format": "#####"
    },
    "MS": {
        "name": "Montserrat",
        "postal_code_format": "#####"
    },
    "MT": {
        "name": "Malta",
        "postal_code_format": "??? ####"
    },
    "MU": {
        "name": "Mauritius",
        "postal_code_format": "#####"
    },
    "MV": {
        "name": "Maldives",
        "postal_code_format": "#####"
    },
    "MW": {
        "name": "Malawi",
        "postal_code_format": "######"
    },
    "MX": {
        "name": "Mexico",
        "postal_code_format": "#####"
    },
    "MY": {
        "name": "Malaysia",
        "postal_code_format": "#####"
    },
    "MZ": {
        "name": "Mozambique",
        "postal_code_format": "####"
    },
    "NA": {
        "name": "Namibia",
        "postal_code_format": "#####"
    },
    "NC": {
        "name": "New Caledonia",
        "postal_code_format": "#####"
    },
    "NE": {
        "name": "Niger",
        "postal_code_format": "####"
    },
    "NF": {
        "name": "Norfolk Island",
        "postal_code_format": "####"
    },
    "NG": {
        "name": "Nigeria",
        "postal_code_format": "######"
    },
    "NI": {
        "name": "Nicaragua",
        "postal_code_format": "###-###-#"
    },
    "NL": {
        "name": "Netherlands",
        "postal_code_format": "#### ??"
    },
    "NO": {
        "name": "Norway",
        "postal_code_format": "####"
    },
    "NP": {
        "name": "Nepal",
        "postal_code_format": "#####"
    },
    "NR": {
        "name": "Nauru",
        "postal_code_format": "#####"
    },
    "NU": {
        "name": "Niue",
        "postal_code_format": "#####"
    },
    "NZ": {
        "name": "New Zealand",
        "postal_code_format": "####"
    },
    "OM": {
        "name": "Oman",
        "postal_code_format": "###"
    },
    "PA": {
        "name": "Panama",
        "postal_code_format": "#####"
    },
    "PE": {
        "name": "Peru",
        "postal_code_format": "#####"
    },
    "PF": {
        "name": "French Polynesia",
        "postal_code_format": "#####"
    },
    "PG": {
        "name": "Papua New Guinea",
        "postal_code_format": "###"
    },
    "PH": {
        "name": "Philippines",
        "postal_code_format": "####"
    },
    "PK": {
        "name": "Pakistan",
        "postal_code_format": "#####"
    },
    "PL": {
        "name": "Poland",
        "postal_code_format": "##-###"
    },
    "PM": {
        "name": "Saint Pierre and Miquelon",
        "postal_code_format": "#####"
    },
    "PN": {
        "name": "Pitcairn",
        "postal_code_format": "#####"
    },
    "PR": {
        "name": "Puerto Rico",
        "postal_code_format": "#####-####"
    },
    "PS": {
        "name": "Palestinian Territory",
        "postal_code_format": "#####"
    },
    "PT": {
        "name": "Portugal",
        "postal_code_format": "####-###"
    },
    "PW": {
        "name": "Palau",
        "postal_code_format": "96940"
    },
    "PY": {
        "name": "Paraguay",
        "postal_code_format": "####"
    },
    "QA": {
        "name": "Qatar",
        "postal_code_format": "#####"
    },
    "RE": {
        "name": "Reunion",
        "postal_code_format": "#####"
    },
    "RO": {
        "name": "Romania",
        "postal_code_format": "######"
    },
    "RS": {
        "name": "Serbia",
        "postal_code_format": "######"
    },
    "RU": {
        "name": "Russia",
        "postal_code_format": "######"
    },
    "RW": {
        "name": "Rwanda",
        "postal_code_format": "#####"
    },
    "SA": {
        "name": "Saudi Arabia",
        "postal_code_format": "#####"
    },
    "SB": {
        "name": "Solomon Islands",
        "postal_code_format": "#####"
    },
    "SC": {
        "name": "Seychelles",
        "postal_code_format": "#####"
    },
    "SD": {
        "name": "Sudan",
        "postal_code_format": "#####"
    },
    "SE": {
        "name": "Sweden",
        "postal_code_format": "### ##"
    },
    "SG": {
        "name": "Singapore",
        "postal_code_format": "######"
    },
    "SH": {
        "name": "Saint Helena",
        "postal_code_format": "STHL 1ZZ"
    },
    "SI": {
        "name": "Slovenia",
        "postal_code_format": "####"
    },
    "SJ": {
        "name": "Svalbard and Jan Mayen",
        "postal_code_format": "####"
    },
    "SK": {
        "name": "Slovakia",
        "postal_code_format": "### ##"
    },
    "SL": {
        "name": "Sierra Leone",
        "postal_code_format": "#####"
    },
    "SM": {
        "name": "San Marino",
        "postal_code_format": "4789#"
    },
    "SN": {
        "name": "Senegal",
        "postal_code_format": "#####"
    },
    "SO": {
        "name": "Somalia",
        "postal_code_format": "??  #####"
    },
    "SR": {
        "name": "Suriname",
        "postal_code_format": "#####"
    },
    "SS": {
        "name": "South Sudan",
        "postal_code_format": "#####"
    },
    "ST": {
        "name": "Sao Tome and Principe",
        "postal_code_format": "#####"
    },
    "SV": {
        "name": "El Salvador",
        "postal_code_format": "CP ####"
    },
    "SX": {
        "name": "Sint Maarten",
        "postal_code_format": "#####"
    },
    "SY": {
        "name": "Syria",
        "postal_code_format": "#####"
    },
    "SZ": {
        "name": "Eswatini",
        "postal_code_format": "?###"
    },
    "TC": {
        "name": "Turks and Caicos Islands",
        "postal_code_format": "TKCA 1ZZ"
    },
    "TD": {
        "name": "Chad",
        "postal_code_format": "#####"
    },
    "TF": {
        "name": "French Southern Territories",
        "postal_code_format": "#####"
    },
    "TG": {
        "name": "Togo",
        "postal_code_format": "#####"
    },
    "TH": {
        "name": "Thailand",
        "postal_code_format": "#####"
    },
    "TJ": {
        "name": "Tajikistan",
        "postal_code_format": "######"
    },
    "TK": {
        "name": "Tokelau",
        "postal_code_format": "#####"
    },
    "TL": {
        "name": "Timor Leste",
        "postal_code_format": "#####"
    },
    "TM": {
        "name": "Turkmenistan",
        "postal_code_format": "######"
    },
    "TN": {
        "name": "Tunisia",
        "postal_code_format": "####"
    },
    "TO": {
        "name": "Tonga",
        "postal_code_format": "#####"
    },
    "TR": {
        "name": "Turkey",
        "postal_code_format": "#####"
    },
    "TT": {
        "name": "Trinidad and Tobago",
        "postal_code_format": "#####"
    },
    "TV": {
        "name": "Tuvalu",
        "postal_code_format": "#####"
    },
    "TW": {
        "name": "Taiwan",
        "postal_code_format": "#####"
    },
    "TZ": {
        "name": "Tanzania",
        "postal_code_format": "#####"
    },
    "UA": {
        "name": "Ukraine",
        "postal_code_format": "#####"
    },
    "UG": {
        "name": "Uganda",
        "postal_code_format": "#####"
    },
    "UM": {
        "name": "United States Minor Outlying Islands",
        "postal_code_format": "#####"
    },
    "US": {
        "name": "United States",
        "postal_code_format": "#####-####"
    },
    "UY": {
        "name": "Uruguay",
        "postal_code_format": "#####"
    },
    "UZ": {
        "name": "Uzbekistan",
        "postal_code_format": "######"
    },
    "VA": {
        "name": "Vatican",
        "postal_code_format": "#####"
    },
    "VC": {
        "name": "Saint Vincent and the Grenadines",
        "postal_code_format": "#####"
    },
    "VE": {
        "name": "Venezuela",
        "postal_code_format": "####"
    },
    "VG": {
        "name": "British Virgin Islands",
        "postal_code_format": "#####"
    },
    "VI": {
        "name": "U.S. Virgin Islands",
        "postal_code_format": "#####-####"
    },
    "VN": {
        "name": "Vietnam",
        "postal_code_format": "######"
    },
    "VU": {
        "name": "Vanuatu",
        "postal_code_format": "#####"
    },
    "WF": {
        "name": "Wallis and Futuna",
        "postal_code_format": "#####"
    },
    "WS": {
        "name": "Samoa",
        "postal_code_format": "#####"
    },
    "XK": {
        "name": "Kosovo",
        "postal_code_format": "#####"
    },
    "YE": {
        "name": "Yemen",
        "postal_code_format": "#####"
    },
    "YT": {
        "name": "Mayotte",
        "postal_code_format": "#####"
    },
    "ZA": {
        "name": "South Africa",
        "postal_code_format": "####"
    },
    "ZM": {
        "name": "Zambia",
        "postal_code_format": "#####"
    },
    "ZW": {
        "name": "Zimbabwe",
        "postal_code_format": "#####"
    }
}

exports.all_countries = all_countries;

exports.get_country_name = (code) => {
    const country = all_countries[code];
    return country ? country.name : "UNKNOWN-COUNTRY";
}

exports.get_zip_code_format = (code) => {
    const country = all_countries[code];
    return country ? country.postal_code_format : "#####";
}

exports.iban_countries = [
    "AL", "AD", "AT", "AZ", "BH", "BY", "BE", "BA", "BR", "VG", "BG", "BI", "CR", "HR", "CY", "CZ", "DK", "DJ",
    "DO", "EG", "SV", "EE", "FO", "FI", "FR", "GE", "DE", "GI", "GR", "GL", "GT", "HU", "IS", "IQ", "IE", "IL",
    "IT", "JO", "KZ", "XK", "KW", "LV", "LB", "LY", "LI", "LT", "LU", "MT", "MR", "MU", "MD", "MC", "ME", "NL",
    "MK", "NO", "PK", "PS", "PL", "PT", "QA", "RO", "RU", "LC", "SM", "ST", "SA", "RS", "SC", "SK", "SI", "ES",
    "SD", "SE", "CH", "TL", "TN", "TR", "UA", "AE", "GB", "VA"
]

exports.all_time_zones = {
    "Africa": [
        "Africa/Abidjan",
        "Africa/Accra",
        "Africa/Addis_Ababa",
        "Africa/Algiers",
        "Africa/Asmara",
        "Africa/Bamako",
        "Africa/Bangui",
        "Africa/Banjul",
        "Africa/Bissau",
        "Africa/Blantyre",
        "Africa/Brazzaville",
        "Africa/Bujumbura",
        "Africa/Cairo",
        "Africa/Casablanca",
        "Africa/Conakry",
        "Africa/Dakar",
        "Africa/Dar_es_Salaam",
        "Africa/Djibouti",
        "Africa/Douala",
        "Africa/El_Aaiun",
        "Africa/Freetown",
        "Africa/Gaborone",
        "Africa/Harare",
        "Africa/Johannesburg",
        "Africa/Juba",
        "Africa/Kampala",
        "Africa/Khartoum",
        "Africa/Kigali",
        "Africa/Kinshasa",
        "Africa/Lagos",
        "Africa/Libreville",
        "Africa/Lome",
        "Africa/Luanda",
        "Africa/Lubumbashi",
        "Africa/Lusaka",
        "Africa/Malabo",
        "Africa/Maputo",
        "Africa/Maseru",
        "Africa/Mbabane",
        "Africa/Mogadishu",
        "Africa/Monrovia",
        "Africa/Nairobi",
        "Africa/Ndjamena",
        "Africa/Niamey",
        "Africa/Nouakchott",
        "Africa/Ouagadougou",
        "Africa/Porto-Novo",
        "Africa/Sao_Tome",
        "Africa/Tripoli",
        "Africa/Tunis",
        "Africa/Windhoek",
        "Atlantic/Cape_Verde",
        "Atlantic/St_Helena",
        "Indian/Antananarivo",
        "Indian/Comoro",
        "Indian/Mahe",
        "Indian/Mauritius",
        "Indian/Mayotte",
        "Indian/Reunion"
    ],
    "Antarctica": [
        "Antarctica/Casey",
        "Antarctica/Davis",
        "Antarctica/DumontDUrville",
        "Antarctica/Mawson",
        "Antarctica/McMurdo",
        "Antarctica/Palmer",
        "Antarctica/Rothera",
        "Antarctica/Syowa",
        "Antarctica/Troll",
        "Antarctica/Vostok",
        "Atlantic/South_Georgia",
        "Indian/Kerguelen"
    ],
    "Asia": [
        "Asia/Aden",
        "Asia/Almaty",
        "Asia/Amman",
        "Asia/Aqtau",
        "Asia/Aqtobe",
        "Asia/Ashgabat",
        "Asia/Atyrau",
        "Asia/Baghdad",
        "Asia/Bahrain",
        "Asia/Baku",
        "Asia/Bangkok",
        "Asia/Beirut",
        "Asia/Bishkek",
        "Asia/Brunei",
        "Asia/Choibalsan",
        "Asia/Colombo",
        "Asia/Damascus",
        "Asia/Dhaka",
        "Asia/Dubai",
        "Asia/Dushanbe",
        "Asia/Gaza",
        "Asia/Hebron",
        "Asia/Ho_Chi_Minh",
        "Asia/Hong_Kong",
        "Asia/Hovd",
        "Asia/Jakarta",
        "Asia/Jayapura",
        "Asia/Jerusalem",
        "Asia/Kabul",
        "Asia/Karachi",
        "Asia/Kathmandu",
        "Asia/Kolkata",
        "Asia/Kuala_Lumpur",
        "Asia/Kuching",
        "Asia/Kuwait",
        "Asia/Macau",
        "Asia/Makassar",
        "Asia/Manila",
        "Asia/Muscat",
        "Asia/Oral",
        "Asia/Phnom_Penh",
        "Asia/Pontianak",
        "Asia/Pyongyang",
        "Asia/Qatar",
        "Asia/Qostanay",
        "Asia/Qyzylorda",
        "Asia/Riyadh",
        "Asia/Samarkand",
        "Asia/Seoul",
        "Asia/Shanghai",
        "Asia/Singapore",
        "Asia/Taipei",
        "Asia/Tashkent",
        "Asia/Tbilisi",
        "Asia/Tehran",
        "Asia/Thimphu",
        "Asia/Tokyo",
        "Asia/Ulaanbaatar",
        "Asia/Urumqi",
        "Asia/Vientiane",
        "Asia/Yangon",
        "Asia/Yerevan",
        "Europe/Istanbul",
        "Indian/Chagos",
        "Indian/Cocos",
        "Indian/Maldives"
    ],
    "Europe": [
        "Africa/Ceuta",
        "Arctic/Longyearbyen",
        "Asia/Anadyr",
        "Asia/Barnaul",
        "Asia/Chita",
        "Asia/Famagusta",
        "Asia/Irkutsk",
        "Asia/Kamchatka",
        "Asia/Khandyga",
        "Asia/Krasnoyarsk",
        "Asia/Magadan",
        "Asia/Nicosia",
        "Asia/Novokuznetsk",
        "Asia/Novosibirsk",
        "Asia/Omsk",
        "Asia/Sakhalin",
        "Asia/Srednekolymsk",
        "Asia/Tomsk",
        "Asia/Ust-Nera",
        "Asia/Vladivostok",
        "Asia/Yakutsk",
        "Asia/Yekaterinburg",
        "Atlantic/Azores",
        "Atlantic/Canary",
        "Atlantic/Faroe",
        "Atlantic/Madeira",
        "Atlantic/Reykjavik",
        "Europe/Amsterdam",
        "Europe/Andorra",
        "Europe/Astrakhan",
        "Europe/Athens",
        "Europe/Belgrade",
        "Europe/Berlin",
        "Europe/Bratislava",
        "Europe/Brussels",
        "Europe/Bucharest",
        "Europe/Budapest",
        "Europe/Busingen",
        "Europe/Chisinau",
        "Europe/Copenhagen",
        "Europe/Dublin",
        "Europe/Gibraltar",
        "Europe/Guernsey",
        "Europe/Helsinki",
        "Europe/Isle_of_Man",
        "Europe/Jersey",
        "Europe/Kaliningrad",
        "Europe/Kirov",
        "Europe/Kyiv",
        "Europe/Lisbon",
        "Europe/Ljubljana",
        "Europe/London",
        "Europe/Luxembourg",
        "Europe/Madrid",
        "Europe/Malta",
        "Europe/Mariehamn",
        "Europe/Minsk",
        "Europe/Monaco",
        "Europe/Moscow",
        "Europe/Oslo",
        "Europe/Paris",
        "Europe/Podgorica",
        "Europe/Prague",
        "Europe/Riga",
        "Europe/Rome",
        "Europe/Samara",
        "Europe/San_Marino",
        "Europe/Sarajevo",
        "Europe/Saratov",
        "Europe/Simferopol",
        "Europe/Skopje",
        "Europe/Sofia",
        "Europe/Stockholm",
        "Europe/Tallinn",
        "Europe/Tirane",
        "Europe/Ulyanovsk",
        "Europe/Vaduz",
        "Europe/Vatican",
        "Europe/Vienna",
        "Europe/Vilnius",
        "Europe/Volgograd",
        "Europe/Warsaw",
        "Europe/Zagreb",
        "Europe/Zurich"
    ],
    "North America": [
        "America/Adak",
        "America/Anchorage",
        "America/Anguilla",
        "America/Antigua",
        "America/Aruba",
        "America/Atikokan",
        "America/Bahia_Banderas",
        "America/Barbados",
        "America/Belize",
        "America/Blanc-Sablon",
        "America/Boise",
        "America/Cambridge_Bay",
        "America/Cancun",
        "America/Cayman",
        "America/Chicago",
        "America/Chihuahua",
        "America/Ciudad_Juarez",
        "America/Costa_Rica",
        "America/Creston",
        "America/Curacao",
        "America/Danmarkshavn",
        "America/Dawson",
        "America/Dawson_Creek",
        "America/Denver",
        "America/Detroit",
        "America/Dominica",
        "America/Edmonton",
        "America/El_Salvador",
        "America/Fort_Nelson",
        "America/Glace_Bay",
        "America/Goose_Bay",
        "America/Grand_Turk",
        "America/Grenada",
        "America/Guadeloupe",
        "America/Guatemala",
        "America/Halifax",
        "America/Havana",
        "America/Hermosillo",
        "America/Indiana/Indianapolis",
        "America/Indiana/Knox",
        "America/Indiana/Marengo",
        "America/Indiana/Petersburg",
        "America/Indiana/Tell_City",
        "America/Indiana/Vevay",
        "America/Indiana/Vincennes",
        "America/Indiana/Winamac",
        "America/Inuvik",
        "America/Iqaluit",
        "America/Jamaica",
        "America/Juneau",
        "America/Kentucky/Louisville",
        "America/Kentucky/Monticello",
        "America/Kralendijk",
        "America/Los_Angeles",
        "America/Lower_Princes",
        "America/Managua",
        "America/Marigot",
        "America/Martinique",
        "America/Matamoros",
        "America/Mazatlan",
        "America/Menominee",
        "America/Merida",
        "America/Metlakatla",
        "America/Mexico_City",
        "America/Miquelon",
        "America/Moncton",
        "America/Monterrey",
        "America/Montserrat",
        "America/Nassau",
        "America/New_York",
        "America/Nome",
        "America/North_Dakota/Beulah",
        "America/North_Dakota/Center",
        "America/North_Dakota/New_Salem",
        "America/Nuuk",
        "America/Ojinaga",
        "America/Panama",
        "America/Phoenix",
        "America/Port-au-Prince",
        "America/Port_of_Spain",
        "America/Puerto_Rico",
        "America/Rankin_Inlet",
        "America/Regina",
        "America/Resolute",
        "America/Santo_Domingo",
        "America/Scoresbysund",
        "America/Sitka",
        "America/St_Barthelemy",
        "America/St_Johns",
        "America/St_Kitts",
        "America/St_Lucia",
        "America/St_Thomas",
        "America/St_Vincent",
        "America/Swift_Current",
        "America/Tegucigalpa",
        "America/Thule",
        "America/Tijuana",
        "America/Toronto",
        "America/Tortola",
        "America/Vancouver",
        "America/Whitehorse",
        "America/Winnipeg",
        "America/Yakutat",
        "America/Yellowknife",
        "Atlantic/Bermuda",
        "Pacific/Honolulu"
    ],
    "Oceania": [
        "Antarctica/Macquarie",
        "Asia/Dili",
        "Australia/Adelaide",
        "Australia/Brisbane",
        "Australia/Broken_Hill",
        "Australia/Darwin",
        "Australia/Eucla",
        "Australia/Hobart",
        "Australia/Lindeman",
        "Australia/Lord_Howe",
        "Australia/Melbourne",
        "Australia/Perth",
        "Australia/Sydney",
        "Indian/Christmas",
        "Pacific/Apia",
        "Pacific/Auckland",
        "Pacific/Bougainville",
        "Pacific/Chatham",
        "Pacific/Chuuk",
        "Pacific/Efate",
        "Pacific/Fakaofo",
        "Pacific/Fiji",
        "Pacific/Funafuti",
        "Pacific/Gambier",
        "Pacific/Guadalcanal",
        "Pacific/Guam",
        "Pacific/Kanton",
        "Pacific/Kiritimati",
        "Pacific/Kosrae",
        "Pacific/Kwajalein",
        "Pacific/Majuro",
        "Pacific/Marquesas",
        "Pacific/Midway",
        "Pacific/Nauru",
        "Pacific/Niue",
        "Pacific/Norfolk",
        "Pacific/Noumea",
        "Pacific/Pago_Pago",
        "Pacific/Palau",
        "Pacific/Pitcairn",
        "Pacific/Pohnpei",
        "Pacific/Port_Moresby",
        "Pacific/Rarotonga",
        "Pacific/Saipan",
        "Pacific/Tahiti",
        "Pacific/Tarawa",
        "Pacific/Tongatapu",
        "Pacific/Wake",
        "Pacific/Wallis"
    ],
    "South America": [
        "America/Araguaina",
        "America/Argentina/Buenos_Aires",
        "America/Argentina/Catamarca",
        "America/Argentina/Cordoba",
        "America/Argentina/Jujuy",
        "America/Argentina/La_Rioja",
        "America/Argentina/Mendoza",
        "America/Argentina/Rio_Gallegos",
        "America/Argentina/Salta",
        "America/Argentina/San_Juan",
        "America/Argentina/San_Luis",
        "America/Argentina/Tucuman",
        "America/Argentina/Ushuaia",
        "America/Asuncion",
        "America/Bahia",
        "America/Belem",
        "America/Boa_Vista",
        "America/Bogota",
        "America/Campo_Grande",
        "America/Caracas",
        "America/Cayenne",
        "America/Cuiaba",
        "America/Eirunepe",
        "America/Fortaleza",
        "America/Guayaquil",
        "America/Guyana",
        "America/La_Paz",
        "America/Lima",
        "America/Maceio",
        "America/Manaus",
        "America/Montevideo",
        "America/Noronha",
        "America/Paramaribo",
        "America/Porto_Velho",
        "America/Punta_Arenas",
        "America/Recife",
        "America/Rio_Branco",
        "America/Santarem",
        "America/Santiago",
        "America/Sao_Paulo",
        "Atlantic/Stanley",
        "Pacific/Easter",
        "Pacific/Galapagos"
    ]
}